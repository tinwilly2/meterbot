...
/**
 * Constructor function. It accepts a settings object which should 
...
/**
 * On message callback, called when a message (of any type) is detected 
with the real time messaging API
...
meterbot.prototype._sendHelloWorld = function (originalMessage) {
    ...
    self.postMessageToChannel(channel.name, textforsending, {as_user: 
true});
};
...
/**
 * Check if the first time the bot is run. It's used to send a welcome 
...
meterbot.prototype._welcomeMessage = function () {
    this.postMessageToChannel(this.channels[0].name, 'Hi guys! Meterbot 
saying hello! ' +
        '\n I do stuff with meter readings. Just say `' + this.name + '` 
to invoke me!',
        {as_user: true});
};

/**
 * Util function to check if a given real time message object represents 
a chat message
 * @param {object} message
 * @returns {boolean}
 * @private
...
/**
 * Util function to check if a given real time message object is 
...
/**
 * Util function to check if a given real time message has ben sent by 
the meterbot
 * @param {object} message
 * @returns {boolean}
 * @private
